...
public class TempRepo extends BaseModel implements ITempRepo {
    ...
    @Override
    public boolean add(TemporaryRegistration reg) throws Exception {
    ...
    public TemporaryRegistration findById(int id) {
        String sql = "SELECT * FROM temporary_registrations WHERE id = ?";
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {
    ...
    private void setCitizenId(TemporaryRegistration reg) {
        String cccd = ResidentModel.getInstance().getResidentById(reg.getPersonId()).getIdentityNumber();
        reg.setCitizenId(cccd);
    }
}
